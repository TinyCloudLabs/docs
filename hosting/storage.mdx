...
<Note>
If you use filesystem-based staging or temp-file + rename flows on a mounted volume, keep `TMPDIR` on the same filesystem as the node data directory so local writes do not cross devices and fail with `EXDEV`.
</Note>
...
For local to S3:
```bash
aws s3 sync ./data/blocks/ s3://tinycloud-blocks/
```
</Step>
