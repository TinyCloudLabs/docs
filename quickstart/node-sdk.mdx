...
<Tabs>
  ...
  <Tab title="Explained">
    ...
    // tc.did returns your owner DID (user identity)
    console.log('DID:', tc.did); // did:pkh:eip155:1:0x...
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Quick">
    ...
    const delResult = await tc.kv.delete('config');
    if (delResult.ok) {
...
<Tabs>
  ...
  <Tab title="Explained">
    ...
    // (since there is no owner identity to authenticate with)
...
<Note>
  After signIn, `tc.did` returns your owner DID (the identity you authenticated with). Before signIn, it returns your session key DID. For delegations, always use the recipient's owner DID (their `tc.did` after they've signed in).
</Note>
...
<Tabs>
  ...
  <Tab title="Explained">
    ...
    const delegation = await alice.createDelegation({
      // Bob's owner DID (tc.did after signIn)
...
|----------|---------------|--------------|-------------------|
| `tc.did` | Session DID | Owner DID | Session DID |
...
<CardGroup cols={2}>
  ...
  <Card title="Web SDK" icon="globe" href="/quickstart/web-sdk">
