...
Create a delegated agent profile once if one does not already exist:

```bash
tc profile create agent --posture delegate-session --operator agent
```

Configure `@tinycloud/mcp@beta` with an explicit delegated profile as described
in [TinyCloud MCP](/cli/mcp). The public beta exploration surface is:
