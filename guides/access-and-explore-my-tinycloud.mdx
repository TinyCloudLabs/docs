...
the same tag and include `byteLength`. Each SQL tool requests read capability
for the exact database, not the entire space.
...
- a SQL request is not one bounded read-only `SELECT`
