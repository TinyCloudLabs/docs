...
Apps that create or alter schema need the `schema` manifest action
(`tinycloud.sql/schema` at the protocol level) for the same database path they
use at runtime.
...
authorization errors if the manifest does not include the `schema` action.
...
- Check that schema-changing SQL has the `schema` action in the manifest.
