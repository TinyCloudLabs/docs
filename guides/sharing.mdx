...
if (result.ok) {
  console.log(result.data.token); // The tc1:... link
}
```
</Tab>
</Tabs>
...
<Note>
`receiveShare` is a **static** method on `TinyCloudWeb`. You do not need to create a TinyCloud instance or call `signIn()` to use it. The link contains all the credentials needed to fetch the data.
</Note>
...
<Frame>
  <img src="/images/diagrams/sharing-flow.svg" alt="Sharing link generation and receive flow" />
</Frame>
...
```json
{
  ...
  "spaceId": "tinycloud:pkh:eip155:1:0xd8dA...6045:my-app",
  "host": "https://node.tinycloud.xyz",
...
if (result.ok) {
  console.log('Share this link:', result.data.token);
  // tc1:eyJ2ZXJz...
}
```
</Step>
...
if (result.ok) {
  ...
  // 'tinycloud:pkh:eip155:1:0xAlice...address:my-app'
} else {
