...
- **Web SDK**: `tc.kv.put / get / list / delete / head`
- **Node SDK**: `tc.kv.put / get / list / delete / head`
...
<CodeGroup>
```typescript Web SDK
const result = await tc.kv.list({ prefix: 'user/' });
if (result.ok) {
...
const docs2025 = await tc.kv.list({ prefix: 'documents/2025/' });
// ['documents/2025/report.json', 'documents/2025/summary.json']

// List all user keys
const userKeys = await tc.kv.list({ prefix: 'user/' });
...
// Get headers only (without fetching the value)
const meta = await tc.kv.head('document');
if (meta.ok) {
  console.log(meta.data.headers.contentType);
  console.log(meta.data.headers.lastModified);
}
```
...
if (!result.ok) {
  ...
  } else if (result.error.code === 'AUTH_UNAUTHORIZED') {
...
async function getDocumentHistory(docId: string) {
  const result = await tc.kv.list({ prefix: `docs/${docId}/history/` });
...
| Delete a value | `tc.kv.delete(key)` | `tc.kv.delete(key)` | Remove a key and its data |
| Get headers | `tc.kv.head(key)` | `tc.kv.head(key)` | Retrieve headers without the value |
