---
title: "DID Formats"
description: "Understanding owner and session DIDs in TinyCloud"
icon: "fingerprint"
---
...
### Owner DID (User Identity)

The **Owner DID** represents the user's persistent identity, derived from the authentication method they used. For Ethereum wallets, this is a `did:pkh` (Public Key Hash) DID:
...
After calling `signIn()`, `tc.did` returns the **owner DID** (persistent identity):

```typescript
await tc.signIn();

console.log(tc.did);
// did:pkh:eip155:1:0xd8dA...6045 (owner DID)
...
<Tab title="Before signIn()">
Before signIn (or in session-only mode with no wallet), `tc.did` returns the **session key DID**:

```typescript
// Before signIn or in session-only mode
console.log(tc.did);
// did:key:z6Mk...#z6Mk...
...
|----------|-------------|---------|
| User-to-user delegations | `tc.did` | Owner DID (after signIn) |
| Identifying a user persistently | `tc.did` | Owner DID (after signIn) |
| Debugging session key issues | `tc.sessionDid` | Session Key DID |
| Logging internal session state | `tc.sessionDid` | Session Key DID |

<Tip>
After `signIn()`, use `tc.did` for delegations. It returns the owner DID, which is what the server expects. Use `tc.sessionDid` only when you need to inspect the ephemeral session key for debugging.
</Tip>

## The Common Mistake

When Alice wants to delegate capabilities to Bob, developers sometimes use the session DID directly instead of the owner identity:
...
```typescript
// CORRECT - use tc.did (which returns owner DID after signIn)
await alice.createDelegation({
  delegateDID: bob.did,  // Owner DID after signIn
  path: "shared/",
  actions: ["tinycloud.kv/get", "tinycloud.kv/put"],
});
```
...
After signIn, the `.did` property returns your owner DID. This is the DID others should use when creating delegations to you. A delegation targeting a session key DID will always fail because:

1. The server expects the owner DID format for the delegatee field
...
2. **Owner DID expected**: The server expects the delegatee to be the owner DID (e.g., `did:pkh:eip155:...` for Ethereum wallets)
...
The owner DID remains constant as long as Bob uses the same authentication method (e.g., same wallet address), making it the correct identifier for persistent user-to-user relationships.
...
// -> did:pkh:eip155:1:0xabc... (owner DID)

console.log("Session key (for debugging):", tc.sessionDid);
// -> did:key:z6Mk...#z6Mk...

// Summary:
// tc.did        = Owner DID after signIn, session DID before signIn
