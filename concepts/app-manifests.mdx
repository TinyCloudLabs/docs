...
| `knowledge` | No | Pointer to an agent-readable knowledge bundle shipped with the app. |
...
Encryption permissions target encryption network URNs, not TinyCloud spaces or
app-relative paths. Use the full network ID as `path`:
...
```json
{
  ...
  "actions": ["decrypt"],
  "description": "Decrypt records encrypted to the user's default encryption network."
}
```

The SDK expands `decrypt` to `tinycloud.encryption/decrypt` and keeps the
