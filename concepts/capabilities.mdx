...
| Ability | Description |
|---------|-------------|
| `tinycloud.namespace/host` | Access a namespace as a participant |
| `tinycloud.namespace/admin` | Full administrative control over a namespace |
...
Abilities can be scoped to specific paths within a namespace:
...
```mermaid
graph TD
    A[Alice - Namespace Owner] -->|delegates| B[Bob - Editor]
...
3. **Path scoping**: Limit access to specific paths rather than entire namespaces
