...
| Ability | Description |
|---------|-------------|
| `tinycloud.encryption/decrypt` | Decrypt envelopes encrypted to a specific encryption network |
| `tinycloud.encryption/network.create` | Create or initialize a specific owner-owned encryption network |

Encryption capabilities are scoped to network URNs such as
`urn:tinycloud:encryption:did:pkh:eip155:1:0x1234...abcd:default`. SDK
manifest entries request `tinycloud.encryption/decrypt`; when the requested
network belongs to the signing user, SDK sign-in can add the separate
`tinycloud.encryption/network.create` grant and create the missing network
after sign-in.
