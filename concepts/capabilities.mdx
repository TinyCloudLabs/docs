...
<AccordionGroup>
  <Accordion title="Expiry Constraint">
    **Child expiry must be less than or equal to parent expiry.**
    ...
    await alice.createDelegation({
      delegateDID: bob.did,
      expiresIn: '48h', // REJECTED: exceeds parent expiry
    });
    ```
  </Accordion>
...
5. **Use the correct DID**: Always use `tc.did` (PKH DID after signIn) for the delegatee (see [DID Formats](/concepts/did-formats))
