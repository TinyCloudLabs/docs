...
<Frame>
  <img src="/images/diagrams/recap-integration.svg" alt="ReCap integration with SIWE flow" />
</Frame>
...
The `tinycloud.` prefix identifies the ReCap ability namespace. The remaining
components name the service and action.

For example, `tinycloud.kv/get` permits a read from the key-value service and
`tinycloud.encryption/decrypt` permits a decrypt operation through the
encryption service. Encryption abilities use network URNs such as
`urn:tinycloud:encryption:did:pkh:eip155:1:0x1234...abcd:default` as their
resource scope rather than an app-relative path.

<Card title="Capability Registry" icon="list-tree" href="/reference/protocol/capabilities">
  Look up every action, status, alias, and implication in the current
  `tinycloud-node` registry.
</Card>

## Path Scoping

Abilities can be scoped to specific key paths within a space. This limits what
a capability holder can access even when the ability itself is allowed.
...
await tc.createDelegation({
  ...
  // Empty path grants access from the root of the space
});
...
<Frame>
  <img src="/images/diagrams/delegation-hierarchy.svg" alt="Delegation hierarchy showing capability delegation chain" />
</Frame>
...
<AccordionGroup>
  <Accordion title="Expiry Constraint">
    **Child expiry must be less than or equal to parent expiry.**
    ...
    await alice.createDelegation({
      ...
      path: "docs/",
      actions: ["tinycloud.kv/get"],
      expiryMs: 48 * 60 * 60 * 1000, // REJECTED: exceeds parent expiry
    });
    ```
  </Accordion>
  ...
  <Accordion title="not_before Constraint">
    **Child not_before must be greater than or equal to parent not_before.**
    ...
    This constraint is enforced when a delegation chain contains a `not_before` value. The high-level SDK helpers shown here do not expose a `notBefore` option.
  </Accordion>
  ...
  <Accordion title="Action Subset Constraint">
    **Child actions must be a subset of parent actions.**
    ...
    await alice.createDelegation({
      delegateDID: bob.did,
      path: "docs/",
      actions: ["tinycloud.kv/list"], // REJECTED: not in parent's set
    });
    ```
  </Accordion>
  ...
  <Accordion title="Path Constraint">
    **Child path must be within parent path.**
    ...
    await alice.createDelegation({
      delegateDID: bob.did,
      path: "/projects/beta/", // REJECTED: not within parent path
      actions: ["tinycloud.kv/get"],
    });
    ```
  </Accordion>
...
<Frame>
  <img src="/images/diagrams/validation-chain.svg" alt="Server-side delegation chain validation flow" />
</Frame>
