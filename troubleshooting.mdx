...
<AccordionGroup>
  ...
  <Accordion title='Session expired'>
    **Symptom**: Operations that previously worked now fail with authentication or session errors. You may see `AUTH_EXPIRED` or `AUTH_UNAUTHORIZED` error codes.
  ...
  <Accordion title='Sub-delegation rejected: expiry exceeds parent'>
    ...
    **Solution**: Set the child expiry to be equal to or earlier than the parent:
    ...
    await tc.createSubDelegation(parentDelegation, {
      delegateDID: bob.did,
      expiryMs: 24 * 60 * 60 * 1000, // Must be before parent's expiry
    });
    ```
...
5. Join the [TinyCloud Telegram](https://t.me/+-_2ufE0JgIcyMjYx) for community support
