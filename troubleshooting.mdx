...
<AccordionGroup>
  <Accordion title='Delegation failed: invalid delegatee'>
    ...
    **Cause**: You are using the session key DID (`tc.sessionDid`) instead of the PKH DID (`tc.did` after `signIn()`) when specifying the delegatee. The server expects the delegatee to be in PKH format (`did:pkh:eip155:{chainId}:{address}`), not session key format (`did:key:z6Mk...`).

    **Solution**: Use `tc.did` for user-to-user delegations. After `signIn()`, this returns the PKH DID:

    ```typescript
    // Wrong - session key DID
    await alice.createDelegation({ delegateDID: bob.sessionDid });

    // Correct - PKH DID (tc.did after signIn)
    await alice.createDelegation({ delegateDID: bob.did });
    ```

    See [DID Formats](/concepts/did-formats) for a detailed explanation of the smart getter behavior.
  </Accordion>
  ...
  <Accordion title='WASM module not found'>
    **Symptom**: Import errors or runtime errors about missing WASM modules when using the SDK. Common messages include `Cannot find module '@tinycloudlabs/sdk-rs'` or `WebAssembly module not found`.

    **Cause**: The WASM package (`@tinycloudlabs/sdk-rs`) was not built before the TypeScript wrappers. The build order matters because the TypeScript packages depend on WASM artifacts that must exist first.
    ...
    **Solution**: Rebuild in the correct order:
    ...
    1. `@tinycloudlabs/sdk-rs` -- produces WASM artifacts
    2. `@tinycloudlabs/node-sdk-wasm` -- TypeScript wrapper (depends on step 1)
    3. `@tinycloudlabs/web-sdk-wasm` -- TypeScript wrapper (depends on step 1)
  ...
  <Accordion title='Sub-delegation rejected: expiry exceeds parent'>
    ...
    **Solution**: Set the child expiry to be equal to or earlier than the parent:
    ...
    await tc.createSubDelegation({
      parentDelegation,
      delegateDID: bob.did,
      expiresIn: '1d', // Must resolve to before parent's expiry
    });
    ```

    <Note>
    When using relative durations like `'7d'`, the SDK calculates the absolute expiry from the current time. If the parent expires in 3 days, using `expiresIn: '7d'` will fail even though 7 days seems reasonable.
    </Note>
  </Accordion>
</AccordionGroup>
